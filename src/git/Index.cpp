...
std::optional<Index> Index::open(const std::filesystem::path &path)
{
	git_index *index;
...
std::optional<Index> Index::create()
{
	git_index *index;
...
int Index::readTree(const Tree &tree)
{
	return git_index_read_tree(index(), tree);
}

std::optional<Tree> Index::writeTree(const Repo &repo)
{
	git_oid oid;
